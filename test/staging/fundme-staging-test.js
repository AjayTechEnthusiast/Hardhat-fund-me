...
developmentChains.includes(network.name)
  ...
  : describe("Fund me", async function () {
      ...
      beforeEach(async function () {
        ...
        // await deployments.fixture("all"); we don't need to deploy because we already deployed
        // smart contract to a test network
        fundMe = await ethers.getContract("FundMe", deployer);
        // MockV3Aggregator = await ethers.getContract(
        //   "MockV3Aggregator",
        //   deployer
        // ); we don't need V3Aggregator because we are in a test net
      });
      describe("Constructor", async function () {
        it("Should sets the aggregator addresses correctly", async function () {
          const response = await fundMe.priceFeed();
          assert.equal(response, "0xD4a33860578De61DBAbDc8BFdb98FD742fA7028e"); //not true
        });
      });
